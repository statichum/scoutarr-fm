...
RUN echo "SHELL=/bin/bash" > /etc/cron.d/scoutarr \
 ...
 && echo "0 3 * * 0 root python3 /app/src/sync_ratings.py > /proc/1/fd/1 2>&1" >> /etc/cron.d/scoutarr \
 && echo "0 */6 * * * root python3 /app/src/scoutarr.py > /proc/1/fd/1 2>&1" >> /etc/cron.d/scoutarr \
